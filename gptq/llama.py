...
def get_llama(name):
    ...
    # model.seqlen = 4096 
    model.seqlen = 2048 # keeps the same with other experiments
    return model

@torch.no_grad()
def llama_sequential(model, dataloader, dev, inps_dev):
    print('Starting ...')
    ...
    # Note: I think this is required for transformers 4.44.0
    try: model.model.rotary_emb = model.model.rotary_emb.to(dev)
    except: pass
    ...
    # Note: I think this is required for transformers 4.44.0
    try: model.model.rotary_emb = model.model.rotary_emb.cpu()
    except: pass
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                # Jerry : removed old quantization grid information, used for packing
                # res = gptq[name].fasterquant(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dataset', type=str, default='red', choices=['red','red_concat','wikitext2', 'wikitext2_nonunif','c4'],
    ...
    if args.wbits < 16:
        ...
        tick = time.time()
        quantizers = llama_sequential(model, dataloader, DEV, inps_dev)
